 ...
 * @param {boolean} props.isFullSize - Determines if the toolbar item should display the caption text.
 * 
    ...
    <Box cursor={"pointer"} _hover={{fontWeight:"Bold", transform: "scale(1.1)"}} className="ToolbarItem" draggable={false}>
      <Link to={`${ToolbarItemSubRoute}`}>
        <Flex gap={3} flexWrap={"nowrap"}>
          ...
          {isFullSize ? (
            <Text color="#FFF0F5" draggable={false}>{ToolbarItemCaption}</Text>
          ) : null}
        </Flex>
      </Link>
    </Box>
...
ToolbarItem.propTypes = {
  ...
  isFullSize: PropTypes.bool.isRequired, // Optional flag to display the caption text.
};
